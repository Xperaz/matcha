...
export const seedDb = async (req: AuthenticatedRequest, res: Response) => {
    try{
        ...
        for (let i = 0; i < MALE_COUNT; i++){
            const firstName = maleFirstName[Math.floor(Math.random() * maleFirstName.length)];
            const lastName = maleLastName[Math.floor(Math.random() * maleLastName.length)];
            const age = Math.floor(Math.random() * 50) + 18
            const cit = city[Math.floor(Math.random() * city.length)];
            const countr = country[Math.floor(Math.random() * country.length)];
            const profilePicture = path.resolve(maleProfilePictures[Math.floor(Math.random() * maleProfilePictures.length)]);
            const interests = Array.from({length: 5}, () => Interests[Math.floor(Math.random() * Interests.length)]);
            const preferences = Preferences[Math.floor(Math.random() * Preferences.length)];
            const gender = "MALE";
            ...
            const pictures_urls = Array.from({length: 6}, () => path.resolve(pictures[Math.floor(Math.random() * pictures.length)]));
            ...
            for (const image of pictures_urls) {
        ...
        for (let i = 0; i < FEMALE_COUNT; i++){
            const firstName = femaleFirstName[Math.floor(Math.random() * femaleFirstName.length)];
            const lastName = femaleLastName[Math.floor(Math.random() * femaleLastName.length)];
            const age = Math.floor(Math.random() * 50) + 18
            const cit = city[Math.floor(Math.random() * city.length)];
            const countr = country[Math.floor(Math.random() * country.length)];
            const profilePicture = path.resolve(femaleProfilePictures[Math.floor(Math.random() * femaleProfilePictures.length)]);
            const interests = Array.from({length: 5}, () => Interests[Math.floor(Math.random() * Interests.length)]);
            const preferences = Preferences[Math.floor(Math.random() * Preferences.length)];
            const gender = "FEMALE";
            const email = `${firstName}${lastName}${i}@gmail.com`;
